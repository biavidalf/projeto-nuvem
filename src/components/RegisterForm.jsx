...
export default function RegisterForm({endpoint, buttonClick, getData, codigoTurma}){
    const url = 'https://backend-server-nuvem.vercel.app/' + endpoint;
    ...
    const getProfessores = () => {
        axios
          .get('https://backend-server-nuvem.vercel.app/professor')
          .then((response) => {
    ...
    const renderPage = () => {
        if(endpoint == 'aluno'){
            ...
            const onSubmit = (e) => {
                ...
                axios.post(url, turma)
                    .then(response => {
                        ...
                        axios.get(`https://backend-server-nuvem.vercel.app/aluno?semestre=${semestre}&curso=${curso}`)
                            .then(response => {
                                ...
                                if(alunosPorCursoSemestre){
                                    for(let aluno in alunosPorCursoSemestre){
                                        ...
                                        axios.put(`https://backend-server-nuvem.vercel.app/aluno/${alunosPorCursoSemestre[aluno].matricula}`, {turmas: turmasAluno})
                                            .catch((error) => {
                                                console.log(error);
                                            });
                                    }
            ...
            const [etapa, setEtapa] = useState('av1');
            
            const onSubmit = async (e) => {
                e.preventDefault();
                let instancias = [];
                await axios.get('https://backend-server-nuvem.vercel.app/aluno?turma='+ codigoTurma)
                    .then(response => {
                ...
                axios.put('https://backend-server-nuvem.vercel.app/turma/' + codigoTurma, {atividade})
                    .then(response => {
